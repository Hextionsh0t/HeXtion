...
$query = "SELECT username, status, rank, level, exp, expneeded, gold, sword, body, legs, helmet, arms FROM users WHERE loginreqkey='$req'";

if ($stmt = $mysqli->prepare($query)) {
    $stmt->execute();
    $stmt->bind_result($gusername, $gstate, $grank, $glevel, $gexp, $gexpn, $ggold, $gsword, $gbody, $glegs, $ghelmet, $garms);
    while ($stmt->fetch()) {
     echo "{";
	 echo '"state": "' . $gstate . '",';
